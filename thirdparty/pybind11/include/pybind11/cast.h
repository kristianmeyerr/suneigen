...
public:
    ...
    bool load(handle src, bool convert) {
        ...
        if (std::is_floating_point<T>::value) {
            ...
            object index;
            if (!PYBIND11_LONG_CHECK(python.ptr())) {  // So: index_check(python.ptr())
                index = reinterpret_steal<object>(PyNumber_Index(python.ptr()));
                if (!index) {
                    PyErr_Clear();
                    if (!convert)
                        return false;
...
public:
    bool load(handle src, bool convert) {
        ...
        if (convert || (std::strcmp("numpy.bool_", Py_TYPE(src.ptr())->tp_name) == 0)) {
            ...
            else if (hasattr(python, PYBIND11_BOOL_ATTR)) {
                res = PyObject_IsTrue(python.ptr());
            }
            #else
...
struct pyobject_caster {
    ...
    bool load(handle src, bool /* convert */) {
        ...
        if (std::is_same<T, str>::value && isinstance<bytes>(python)) {
            PyObject *str_from_bytes = PyUnicode_FromEncodedObject(python.ptr(), "utf-8", nullptr);
