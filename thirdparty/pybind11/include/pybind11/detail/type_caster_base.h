...
public:
    ...
    PYBIND11_NOINLINE bool load_impl(handle src, bool convert) {
        ...
        // Case 1: If python is an exact type match for the target type then we can reinterpret_cast
...
// polymorphic_type_hook<itype>::get(python, tinfo) determines whether the object pointed
// to by `python` actually is an instance of some class derived from `itype`.
// If so, it sets `tinfo` to point to the std::type_info representing that derived
// type, and returns a pointer to the start of the most-derived object of that type
// (in which `python` is a subobject; this will be the same address as `python` in most
// single inheritance cases). If not, or if `python` is nullptr, it simply returns `python`
// and leaves `tinfo` at its default value of nullptr.
//
// The default polymorphic_type_hook just returns python. A specialization for polymorphic
